...
const Sidebar: React.FC = () => {
  ...
  const linkClassName = (path: string) =>
    `p-3 m-2 flex justify-center ${
      isActive(path)
        ? "bg-gray-700 dark:bg-gray-200"
        : "hover:bg-gray-700 hover:dark:bg-gray-200"
    }`;
  ...
  return (
    <div className="min-h-screen w-20 bg-gray-800 shadow-lg dark:bg-white text-white flex flex-col items-center">
      ...
      <nav className="flex flex-col flex-grow mt-6 dark:border-1 dark:text-black">
        <Link
          href="/dashboard/home"
          className={`${linkClassName("/dashboard/home")} p-2 mb-4 shadow-lg`}
        >
          <AiTwotoneHome size={24} />
        </Link>
        <Link
          href="/dashboard/search"
          className={`${linkClassName("/dashboard/search")} p-2 mb-4 shadow-lg`}
        >
        ...
        <Link
          href="/dashboard/send"
          className={`${linkClassName("/dashboard/send")} p-2 mb-4 shadow-lg`}
        >
          <IoIosSend size={24} />
        </Link>
        <Link
          href="/dashboard/menu"
          className={`${linkClassName("/dashboard/menu")} p-2 mb-4 shadow-lg`}
        >
        ...
        <div className="relative  shadow-lg">
          <Link
            ...
            className={linkClassName("/dashboard/Email")}
          >
            <FaInbox size={24} />
            <span className="absolute top-1 right-0 flex items-center justify-center w-5 h-5 bg-red-600 text-white text-xs font-bold rounded-md -translate-x-1/2 -translate-y-1/2">
              12+
            </span>
          </Link>
        </div>
        <Link
          ...
          className={`${linkClassName("/dashboard/stats")} p-2 mb-4 shadow-lg`}
        >
          <IoIosStats size={24} />
        </Link>
      </nav>
